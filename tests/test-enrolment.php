...
class Test_Enrolment extends WP_UnitTestCase {
	public function test_first_method_forces_backup_code_generation(): void {
		...
		\Easy2FA\Enrolment::instance()->complete_method( $uid, 'totp', [ 'secret' => 'x' ] );
		$this->assertArrayHasKey( 'backup', \Easy2FA\Store::methods( $uid ) );
	}

	public function test_cannot_remove_last_method_while_required(): void {
		\Easy2FA\Policy::update( [ 'enabled' => true, 'roles' => [ 'administrator' => true ] ] );
		$uid = self::factory()->user->create( [ 'role' => 'administrator' ] );
		wp_set_current_user( $uid );
		\Easy2FA\Store::set_method( $uid, 'totp', [ 'secret' => 'x' ] );
		$this->assertWPError( \Easy2FA\Enrolment::instance()->remove_method( $uid, 'totp' ) );
	}
	...
	public function test_user_cannot_edit_another_users_methods(): void {
		...
		$this->assertWPError( \Easy2FA\Enrolment::instance()->remove_method( $b, 'totp' ) );
	}
}
